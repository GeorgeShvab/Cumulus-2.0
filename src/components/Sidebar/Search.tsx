...
import useCurrentLocation from "@/components/Sidebar/useCurrentLocation";
...
const Search: FC = () => {
      ...
      suggestions.data.filter((item) =>
        item.types.every(
          (value) =>
            value !== "route" &&
            value !== "country" &&
            !value.includes("administrative_area_level")
        )
      ),
    [suggestions.data]
  );
  ...
  const handleSubmit = (
    ...
    const isCoordinates =
      ...
      !isNaN(Number(query.split(" ")[0])) &&
      !isNaN(Number(query.split(" ")[1]));

    if (isCoordinates) {
      router.push(
        `/coordinates?latitude=${query.split(" ")[0]}&longitude=${
          query.split(" ")[1]
        }`
      );
    } else {
    ...
    <form onSubmit={handleSubmit} className="relative" ref={formRef}>
      ...
      <div
        className={`flex gap-3 items-center bg-neutral-100 p-2 ${
          data.length
            ? "rounded-t-lg outline outline-1 outline-b-0 outline-neutral-200 shadow"
            : "rounded-lg"
        }`}
      >
        <button
